...
class StudentTableForm extends FormBase {
  ...
  function get_students($opt) {
    ...
    if($opt == "All"){
      ...
      $results->fields('st');
      // fetch only 15 rows to show on each pagination of table.
      $results->range(0, 15);
    ...
    foreach ($res as $row) {
      ...
      //$edit and $delete are Drupal\Core\Url objects.
	    $edit_link = Link::fromTextAndUrl(t('Edit'), $edit);
	    $delete_link = Link::fromTextAndUrl(t('Delete'), $delete);
      //you are converting Drupal URLs into renderable arrays. This is useful when you want to include these links in a render array for rendering in a Drupal page.
      ...
      // Create a render array with the links.
      $ret[] = [
        'id' => $row->id,
        'fname' => $row->fname,
	  	  'sname' => $row->sname,
